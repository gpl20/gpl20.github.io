...
function loadSaveBallPositions() {
    ...
    if (keys.Digit1) {
        keys.Digit1 = false;
        positionSaves.length > 10 && positionSaves.shift();
...
function doMouseInterface() {
    ...
    if (autoStart) {
        autoTime ++;
        if (autoTime < 30) {
            mouse.tx = B.x - autoTime * TABLE_SCALE* 2;
            mouse.ty = B.y + 1 * TABLE_SCALE;
            if (autoTime > 20) {
                mouse.button = 1;
            }
            
        } else if(autoTime < 40) {
            mouse.tx = B.x - 30 * TABLE_SCALE* 2;
            mouse.ty = B.y + 1 * TABLE_SCALE;;
            mouse.button = 0;
        } else {
            //autoStart = false;
        ...
        if ((mouse.button & 1) === 1) {
            ...
            if (mouse.speed === 0) {
                if (!placeBalls && (mouse.button & 4) === 4) {
                    message = "Place mode. Click ball to move it, click agian to place";
                    messages.length = 0;
                    messages.push("To exit place mode, place the white ball where you want.",
            ...
            if (mouse.pos < 0) {
                B.vx = (Math.cos(mouse.angleToHit + Math.PI) * mouse.speed * mouse.mass) / BALL_MASS;
                B.vy = (Math.sin(mouse.angleToHit + Math.PI) * mouse.speed * mouse.mass) / BALL_MASS;
                mouse.pull = 0;
                mouse.spring = 0;
                mouse.speed = 0;
                mouse.pos = 0;
